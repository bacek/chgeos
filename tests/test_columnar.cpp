...
// ── PrepCache: few-distinct WKB columns ──────────────────────────────────────
// Simulates the Q6 cross-join pattern: col(1) has 2 distinct zone polygons
// interleaved with col(0) trip points.  PrepCache should build 2 PreparedGeometry
// objects and reuse them, producing the same results as the baseline path.

TEST(ColumnarPrepCache, BCache_FewDistinct_MatchesBaseline) {
    // Two zone polygons alternating as col(1), points as col(0).
    auto zone1 = wkt2wkb("POLYGON ((0 0, 1 0, 1 1, 0 1, 0 0))");
    auto zone2 = wkt2wkb("POLYGON ((2 0, 3 0, 3 1, 2 1, 2 0))");
    auto pt_z1 = wkt2wkb("POINT (0.5 0.5)");   // inside zone1, outside zone2
    auto pt_z2 = wkt2wkb("POINT (2.5 0.5)");   // outside zone1, inside zone2
    auto pt_out= wkt2wkb("POINT (5.0 5.0)");   // outside both

    // 6 rows: each point paired with alternating zones (simulates cross join output)
    //   row 0: pt_z1 × zone1 → true
    //   row 1: pt_z1 × zone2 → false
    //   row 2: pt_z2 × zone1 → false
    //   row 3: pt_z2 × zone2 → true
    //   row 4: pt_out × zone1 → false
    //   row 5: pt_out × zone2 → false
    const uint32_t n = 6;
    auto* buf_cache = make_columnar(n, {
        bytes_col(false, {pt_z1, pt_z1, pt_z2, pt_z2, pt_out, pt_out}),
        bytes_col(false, {zone1, zone2, zone1, zone2, zone1, zone2}),
    });
    auto got = read_bool_col(
        columnar_impl_wrapper(buf_cache, n, st_within_impl,
            bbox_op_rcontains, false, prep_a_st_within, prep_b_st_within),
        n);
    clickhouse_destroy_buffer(reinterpret_cast<uint8_t*>(buf_cache));

    EXPECT_EQ(got[0], 1u);  // pt_z1 within zone1
    EXPECT_EQ(got[1], 0u);  // pt_z1 not within zone2
    EXPECT_EQ(got[2], 0u);  // pt_z2 not within zone1
    EXPECT_EQ(got[3], 1u);  // pt_z2 within zone2
    EXPECT_EQ(got[4], 0u);  // pt_out not within zone1
    EXPECT_EQ(got[5], 0u);  // pt_out not within zone2
}

TEST(ColumnarPrepCache, ACache_FewDistinct_MatchesBaseline) {
    // col(0) has 2 distinct zone polygons, col(1) varies — A-cache path.
    auto zone1 = wkt2wkb("POLYGON ((0 0, 1 0, 1 1, 0 1, 0 0))");
    auto zone2 = wkt2wkb("POLYGON ((2 0, 3 0, 3 1, 2 1, 2 0))");
    auto pt_in1= wkt2wkb("POINT (0.5 0.5)");
    auto pt_in2= wkt2wkb("POINT (2.5 0.5)");
    auto pt_out= wkt2wkb("POINT (5.0 5.0)");

    // st_contains(zone, point): A-cache on col(0) = zones
    //   row 0: zone1 contains pt_in1 → true
    //   row 1: zone2 contains pt_in1 → false
    //   row 2: zone1 contains pt_in2 → false
    //   row 3: zone2 contains pt_in2 → true
    //   row 4: zone1 contains pt_out → false
    //   row 5: zone2 contains pt_out → false
    const uint32_t n = 6;
    auto* buf = make_columnar(n, {
        bytes_col(false, {zone1, zone2, zone1, zone2, zone1, zone2}),
        bytes_col(false, {pt_in1, pt_in1, pt_in2, pt_in2, pt_out, pt_out}),
    });
    auto got = read_bool_col(
        columnar_impl_wrapper(buf, n, st_contains_impl,
            bbox_op_contains, false, prep_a_st_contains, prep_b_st_contains),
