...
run "Q1" \
"SELECT t_tripkey, st_\1(t_pickuploc), st_\1(t_pickuploc), t_pickuptime,
    st_distanc\1(t_pickuploc, st_geomfromtext('POINT (-111.7610 34.8697)')) AS distance_to_center
 FROM ${TRIP}
 WHERE st_dwithi\1(t_pickuploc, st_geomfromtext('POINT (-111.7610 34.8697)'), 0.45)
...
###  FROM ${TRIP} t
###  WHERE st_intersect\1(t.t_pickuploc,
...
"SELECT z.z_zonekey, z.z_name, count() AS trip_count
 ...
 JOIN (SELECT t_pickuploc FROM ${TRIP} ORDER BY t_tip DESC, t_tripkey ASC LIMIT 1000) top_trips
   ON st_withi\1(top_trips.t_pickuploc, z.z_boundary)
...
"SELECT c.c_custkey, c.c_name AS customer_name,
    ...
    st_are\1(st_convexhul\1(st_collect_agg(t.t_dropoffloc))) AS monthly_travel_hull_area,
...
"SELECT z.z_zonekey, z.z_name,
 ...
 FROM ${TRIP} t, ${ZONE} z
 WHERE st_intersect\1(st_geomfromtext('POLYGON((-112.2110 34.4197,-111.3110 34.4197,-111.3110 35.3197,-112.2110 35.3197,-112.2110 34.4197))'), z.z_boundary)
   AND st_withi\1(t.t_pickuploc, z.z_boundary)
 GROUP BY z.z_zonekey, z.z_name
...
"WITH trip_lengths AS (
     SELECT t_tripkey,
         ...
         st_lengt\1(st_makelin\1(t_pickuploc, t_dropoffloc)) / 0.000009 AS line_distance_m
     FROM ${TRIP}
 )
...
"SELECT b.b_buildingkey, b.b_name, count() AS nearby_pickup_count
 FROM ${TRIP} t
 JOIN ${BUILDING} b ON st_dwithi\1(t.t_pickuploc, b.b_boundary, 0.0045)
...
"WITH b1 AS (SELECT b_buildingkey AS id, b_boundary AS geom FROM ${BUILDING}),
      ...
      pairs AS (
          SELECT b1.id AS building_1, b2.id AS building_2,
              st_are\1(b1.geom) AS area1, st_are\1(b2.geom) AS area2,
              st_are\1(st_intersectio\1(b1.geom, b2.geom)) AS overlap_area
          FROM b1 JOIN b2 ON b1.id < b2.id AND st_intersect\1(b1.geom, b2.geom)
      )
...
"SELECT z.z_zonekey, z.z_name AS pickup_zone,
 ...
 FROM ${ZONE} z
 LEFT JOIN ${TRIP} t ON st_withi\1(t.t_pickuploc, z.z_boundary)
 GROUP BY z.z_zonekey, z.z_name
...
"SELECT count() AS cross_zone_trip_count
 FROM ${TRIP} t
 JOIN ${ZONE} pickup_zone  ON st_withi\1(t.t_pickuploc,   pickup_zone.z_boundary)
 JOIN ${ZONE} dropoff_zone ON st_withi\1(t.t_dropoffloc, dropoff_zone.z_boundary)
...
"WITH candidates AS (
     SELECT t.t_tripkey, t.t_pickuploc, b.b_buildingkey, b.b_name AS building_name,
         st_distanc\1(t.t_pickuploc, b.b_boundary) AS distance_to_building,
         row_number() OVER (PARTITION BY t.t_tripkey ORDER BY st_distanc\1(t.t_pickuploc, b.b_boundary)) AS rn
     FROM ${TRIP} t CROSS JOIN ${BUILDING} b
     WHERE st_dwithi\1(t.t_pickuploc, b.b_boundary, 0.05)
 )
